import { endSessionPayload } from "@/lib/memory-service/sessions"
...
const ENDPOINT = "/api/sdk/v1/sessions/end"

const requestSchema = z.object({
  sessionId: z.string().trim().min(1).max(120),
  status: z.enum(["closed", "compacted"]).optional(),
  scope: scopeSchema,
})
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const payload = await endSessionPayload({
      turso,
      args: {
