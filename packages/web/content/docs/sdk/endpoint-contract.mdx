...
Lifecycle hint fields used by `@memories.sh/core` such as `sessionId`, token budgets, inactivity
thresholds, and `includeSessionSummary` are evaluated client-side. They are not part of the raw
HTTP body contract for this endpoint.
