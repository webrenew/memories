...
1. Use your MCP API key as usual (`Authorization: Bearer mcp_...`).
...
### `GET /api/sdk/v1/management/tenants` (legacy: `/api/mcp/tenants`)

List all tenant mappings for your active MCP API key.

### `POST /api/sdk/v1/management/tenants` (legacy: `/api/mcp/tenants`)
...
### `DELETE /api/sdk/v1/management/tenants?tenantId=...` (legacy: `/api/mcp/tenants`)
...
When you rotate your MCP API key, tenant mappings are copied from the old key hash to the new key hash automatically. This keeps `tenant_id` routing working without manual remapping.
