...
export const generateCommand = new Command("generate")
  ...
  .action(async (opts: { types?: string; dryRun?: boolean; force?: boolean; watch?: boolean }) => {
    try {
      ...
      // Handle agents target separately — uses directory generator
      if (selectedSet.has("agents")) {
      ...
      if (adapterTargets.length > 0) {
        // Generate .agents/ once for all adapter targets
      ...
      new Command("agents")
        ...
        .action(async (opts: { dryRun?: boolean }) => {
          try {
            ...
            console.error(chalk.red("✗") + " Failed to generate .agents/:", error instanceof Error ? error.message : "Unknown error");
            process.exit(1);
          }
        }),
    );
