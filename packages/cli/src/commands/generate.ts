...
import { detectTools, toolSupportsGeneration, type Tool } from "../lib/setup.js";
import {
  ADAPTER_TARGETS,
  runAdapter,
  logAdaptResult,
...
function isInstructionOnlyTool(tool: Tool): boolean {
  if (!tool.instructionFile) return false;
  if ((tool.detectCommands?.length ?? 0) > 0) return false;
  if ((tool.globalDetectPaths?.length ?? 0) > 0) return false;
  if (tool.mcpConfigPath || tool.globalMcpConfigPath) return false;

  const nonInstructionPaths = tool.detectPaths.filter((path) => path !== tool.instructionFile);
  return nonInstructionPaths.length === 0;
}

function getAutoDetectedTargetNames(cwd: string = process.cwd()): Set<string> {
  const validTargetNames = new Set(TARGETS.map((target) => target.name));
  const targetNames = new Set<string>();

  for (const detected of detectTools(cwd)) {
    const tool = detected.tool;
    if (!toolSupportsGeneration(tool)) continue;
    if (isInstructionOnlyTool(tool)) continue;
    if (!validTargetNames.has(tool.generateCmd)) continue;
    targetNames.add(tool.generateCmd);
  }

  // Safe fallback when no integrations are detected.
  if (targetNames.size === 0 && validTargetNames.has("agents")) {
    targetNames.add("agents");
  }

  return targetNames;
}

function resolveTargetsByName(targetNames: Set<string>): Target[] {
  return TARGETS.filter((target) => targetNames.has(target.name));
}
...
export const generateCommand = new Command("generate")
  .description("Generate IDE rule/instruction files from stored memories (does not add memories)")
  ...
  .action(async (opts: { types?: string; dryRun?: boolean; force?: boolean; watch?: boolean }) => {
    try {
      ...
      const autoDetectedTargetNames = getAutoDetectedTargetNames(process.cwd());

      const selected = await checkbox({
        message: "Select targets to generate (defaults to detected integrations)",
        choices: TARGETS.map((t) => ({
          name: `${t.name} ${chalk.dim(`→ ${t.defaultPath}`)}`,
          value: t.name,
          checked: autoDetectedTargetNames.has(t.name),
        })),
      });
      ...
      const selectedTargets = TARGETS.filter((t) => selected.includes(t.name));
      const selectedSet = new Set(selectedTargets.map((t) => t.name));
      ...
      for (const target of selectedTargets.filter((t) => selectedSet.has(t.name))) {
        await writeTarget(target, memories, opts);
      }

      if (opts.watch) {
        await runWatch(selectedTargets, () => fetchMemories(types), opts);
      }
    } catch (error) {
...
generateCommand.addCommand(
  new Command("all")
    .description("Generate rule files for detected targets")
    .option("--types <types>", "Comma-separated types to include (default: rule,decision,fact)")
    .option("--dry-run", "Preview without writing")
    .option("--force", "Overwrite files not generated by memories.sh")
    .option("-w, --watch", "Watch for memory changes and auto-regenerate")
    .action(async (opts: { types?: string; dryRun?: boolean; force?: boolean; watch?: boolean }) => {
      try {
        const types = parseTypes(opts.types);
        const selectedTargets = resolveTargetsByName(getAutoDetectedTargetNames(process.cwd()));
        const selectedSet = new Set(selectedTargets.map((target) => target.name));
        ...
        // Step 1: Generate .agents/ directory once when required
        const needsAgents = selectedSet.has("agents") || [...selectedSet].some((n) => ADAPTER_TARGETS.has(n));
        if (needsAgents) {
          await runAgentsGenerator(opts);
        }

        // Step 2: Run adapters for each detected target
